...
export default function newScream(config) {
	const scream = {};
	...
	scream.getMinimalViewHeight = () => {
		...
		let spec = scream.deviceSpec();
		
		if (!spec) {
			console.log('Not a known iOS device.');
			spec = [1280, 2114, 2272, 1280, 320, 568, 2, 'iPhone 5 or 5s'];
	...
	scream.detectViewChange = () => {
		...
		if (!scream.deviceSpec()) {
			...
			console.log('View change detection has been disabled. Unrecognized device.');
			/* eslint-enable */
