...
export default paper = function (self, undefined) {
	...
	var Rectangle = Base.extend({
		...
		initialize: function Rectangle(arg0, arg1, arg2, arg3) {
			...
			if (read === undefined) {
				const frm = Point.readNamed(arguments, 'from'),
				...
				const filtered = arguments.__filtered;
				if (filtered)
	...
	var Item = Base.extend(Emitter, {
		...
		new function () {
			...
			transform: function (matrix, _applyMatrix, _applyRecursively,
			                     _setApplyMatrix) {
				const _matrix = this._matrix;
				let transform = matrix && !matrix.isIdentity(),
				...
				const decomp = transform && bounds && matrix.decompose();
	...
	const Segment = Base.extend({
		...
		initialize: function Segment(arg0, arg1, arg2, arg3, arg4, arg5) {
			...
			let point, handleIn, handleOut, selection;
			if (count > 0) {
				...
				getMonoCurves: function (v, dir) {
					...
					if ((o0 >= o1) === (o1 >= o2) && (o1 >= o2) === (o2 >= o3)
						...
						const a = 3 * (o1 - o2) - o0 + o3,
							...
							tMax = 1 - tMin,
							roots = [];
						let n = Numerical.solveQuadratic(a, b, c, roots, tMin, tMax);
						if (!n) {
							curves.push(v);
						}
						else {
	...
	var CurveLocation = Base.extend({
			...
			equals: function (loc, _ignoreOther) {
				...
				if (!res && loc instanceof CurveLocation) {
					...
					if (p1 === p2) {
						...
						let i1 = !_ignoreOther && this._intersection,
			...
			isCrossing: function () {
				...
				let offset = Math.min.apply(Math, offsets);
		...
		compare: function (path) {
			...
			if (path) {
				...
				for (let i1 = length1 - 1; i1 >= 0 && ok; i1--) {
					...
					ok = false;
					for (let i2 = length2 - 1; i2 >= 0 && !ok; i2--) {
		...
		new function () {
			...
			statics: {
				...
				_addBevelJoin: function (segment, join, radius, miterLimit, matrix,
					...
					let normal1 = curve1.getNormalAtTime(1).multiply(radius)
			...
			for (let i = 0; i < offsets.length && winding.quality < 0.5; i++) {
				...
				for (var j = 0, l = chain.length; j < l; j++) {
					...
					if (length <= curveLength) {
						...
						const wind = !(operator.subtract && path2 && (
							operand === path1 &&
	...
	new function () {
		...
		function getDefinition(value) {
			const match = value && value.match(/\((?:["'#]*)([^"')]+)/),
				name = match && match[1];
			let res = name && definitions[window
		...
		function importSVG(source, options, owner) {
			...
			function onLoad(svg) {
				try {
					let node = typeof svg === 'object' ? svg : new self.DOMParser()
	...
	Base.exports.PaperScript = function () {
		...
		if (!acorn) {
			...
			(function (root, mod) {
				...
				const _break = {keyword: "break"}, _case = {keyword: "case", beforeExpr: true},
					_catch = {keyword: "catch"};
				const _continue = {keyword: "continue"}, _debugger = {keyword: "debugger"},
					_default = {keyword: "default"};
				const _do = {keyword: "do", isLoop: true}, _else = {keyword: "else", beforeExpr: true};
				const _finally = {keyword: "finally"}, _for = {keyword: "for", isLoop: true},
					_function = {keyword: "function"};
				const _if = {keyword: "if"}, _return = {keyword: "return", beforeExpr: true},
				...
				const _bracketL = {type: "[", beforeExpr: true}, _bracketR = {type: "]"},
				...
				const _colon = {type: ":", beforeExpr: true}, _dot = {type: "."},
				...
				function readNumber(startsWithDot) {
					const start = tokPos;
					let isFloat = false, octal = input.charCodeAt(tokPos) === 48;
				...
				function readWord1() {
					...
					for (; ;) {
						...
						if (isIdentifierChar(ch)) {
							...
							let escStr = String.fromCharCode(esc);
